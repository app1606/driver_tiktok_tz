...
# gets all landmarks from db and sends them
def get_all_landmarks(response: Response):
    ...
    cursor.execute("""
        ...
        """)
    landmarks_dict = {
        i + 1: {'longitude': rec[2], 'latitude': rec[1]} for i, rec in enumerate(cursor)}
    conn.commit()
    conn.close()
...
# adds landmark to database with latitude and longitude given
...
# deletes landmark to database with latitude and longitude given
def add_landmark(long: float, lat: float):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("""
